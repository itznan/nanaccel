...
fn run_subtitle_operation(
    ...
    match operation {
        ...
        "burn" => {
            ...
            let escaped_sub_path = sub_path.replace("\\", "/").replace(":", "\\:");
    ...
    if operation == "burn" {
        println!(
            "\x1b[33mWarning: GPU-accelerated burn-in failed. Falling back to software encoder (libx264)...\x1b[0m"
        );
        let mut sw_args = vec![];
        ...
        let sw_status = Command::new("ffmpeg")
            ...
            .map_err(|e| {
                format!(
                    "Failed to execute software fallback ffmpeg for subtitle operation: {}",
                    e
                )
            })?;
